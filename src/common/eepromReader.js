...
try {
    ...
    isDirectRun = invoked && invoked === thisFile;
} catch (_) {
    // 忽略
}

if (isDirectRun) {
    if (process.env.DEBUG_EEPROM) {
        console.error('[EEPROM] CLI start', { DEFAULT_BUS });
    }
    (async () => {
        try {
            const meta = await readEepromJson({ busNumber: DEFAULT_BUS, raw: false, returnMeta: true });
            if (process.env.DEBUG_EEPROM) {
                console.error('[EEPROM] read result', meta);
            }
            if (meta && meta.present) {
                console.log(JSON.stringify({ present: true, bus: DEFAULT_BUS, addr: '0x50', json: meta.json }, null, 2));
                process.exit(0);
            }
            console.log(JSON.stringify({ present: false, bus: DEFAULT_BUS, addr: '0x50' }));
            process.exit(0);
        } catch (err) {
            if (process.env.DEBUG_EEPROM) {
                console.error('[EEPROM] error object', err);
            }
            if (err && MISSING_DEVICE_CODES.includes(err.code)) {
                console.log(JSON.stringify({ present: false }));
                process.exit(0);
            }
            if (err && err.code === 'ETIMEDOUT') {
                console.log(JSON.stringify({ present: false, reason: 'timeout' }));
                process.exit(0);
            }
            console.error('EEPROM_READ_ERROR', err.message || err);
            process.exit(1);
        }
    })();
}
// 如果判断失败但用户期望直接运行，可通过强制变量执行
if (process.env.FORCE_EEPROM_RUN === '1') {
    if (process.env.DEBUG_EEPROM) console.error('[EEPROM] FORCE_EEPROM_RUN triggered');
    (async () => {
        try {
            const meta = await readEepromJson({ busNumber: DEFAULT_BUS, raw: false, returnMeta: true });
            console.log(JSON.stringify(meta.present ? { present: true, bus: DEFAULT_BUS, json: meta.json } : { present: false, bus: DEFAULT_BUS }, null, 2));
        } catch (e) {
            console.error('EEPROM_READ_ERROR', e.message || e);
            process.exit(1);
        }
    })();
}
